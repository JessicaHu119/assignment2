...
metafor::funnel(x = lnRR$lnRR, vi = lnRR$V_lnRR, yaxis = "seinv", xlim = c(-2,3), ylim = c(0.1,100),
    digits = 2, level = c(0.1, 0.05, 0.01), shade = c("white", "gray55", "gray 75"),
    las = 1, xlab = "Correlation Coefficient (r)", atransf = tanh, legend = TRUE, col="skyblue")
```
...
```{r}
rma.mv(lnRR ~ (1 / V_lnRR), V = V_lnRR, random = list(~1 | Study, ~1 | residual), test = "t", dfs = "contain", data = lnRR)
```
